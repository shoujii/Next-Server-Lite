...
NextcloudDBName=$(grep -Pom 1 "(?<=^NextcloudDBName = ).*$" /root/NeXt-Server-Lite/nextcloud_login_data.txt)
NextcloudDBUser=$(grep -Pom 1 "(?<=^NextcloudDBUser = ).*$" /root/NeXt-Server-Lite/nextcloud_login_data.txt)
...
rm /root/NeXt-Server-Lite/nextcloud_login_data.txt
...
sed_replace_word "$NEXTCLOUD_PATH_NAME" "" "/root/NeXt-Server-Lite/configs/blocked_paths.conf"
sed_replace_word "NEXTCLOUD_PATH_NAME=\".*"\" "NEXTCLOUD_PATH_NAME=\"0"\" "/root/NeXt-Server-Lite/configs/userconfig.cfg"
sed_replace_word "NEXTCLOUD_IS_INSTALLED=\"1"\" "NEXTCLOUD_IS_INSTALLED=\"0"\" "/root/NeXt-Server-Lite/configs/userconfig.cfg"
}