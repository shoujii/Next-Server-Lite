...
cp /root/NeXt-Server-Lite/configs/pma/config.inc.php /var/www/${MYDOMAIN}/public/${PHPMYADMIN_PATH_NAME}/config.inc.php
sed_replace_word "PMA_BFSECURE_PASS" "${PMA_BFSECURE_PASS}" "/var/www/${MYDOMAIN}/public/${PHPMYADMIN_PATH_NAME}/config.inc.php"

cp /root/NeXt-Server-Lite/addons/vhosts/_phpmyadmin.conf /etc/nginx/_phpmyadmin.conf
...
touch /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "--------------------------------------------" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "phpmyadmin" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "--------------------------------------------" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "https://${MYDOMAIN}/${PHPMYADMIN_PATH_NAME}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "PMA_HTTPAUTH_USER = ${PMA_HTTPAUTH_USER}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "PMA_HTTPAUTH_PASS = ${PMA_HTTPAUTH_PASS}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "MYSQL_USERNAME: root" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "MYSQL_ROOT_PASS: $MYSQL_ROOT_PASS" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "MYSQL_PMADB_USER = ${MYSQL_PMADB_USER}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "MYSQL_PMADB_NAME = ${MYSQL_PMADB_NAME}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "PMADB_PASS = ${PMADB_PASS}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
echo "blowfish_secret = ${PMA_BFSECURE_PASS}" >> /root/NeXt-Server-Lite/phpmyadmin_login_data.txt

sed_replace_word "PMA_IS_INSTALLED=\"0"\" "PMA_IS_INSTALLED=\"1"\" "/root/NeXt-Server-Lite/configs/userconfig.cfg"
echo "${PHPMYADMIN_PATH_NAME}" >> /root/NeXt-Server-Lite/configs/blocked_paths.conf

systemctl -q restart php$PHPVERSION8-fpm.service
systemctl -q reload nginx.service

dialog_msg "Please save the shown login information on next page"
cat /root/NeXt-Server-Lite/phpmyadmin_login_data.txt
continue_or_exit
}